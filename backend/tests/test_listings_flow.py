...
class CurrencyTests(TestCase):
    """plan2 §6: الرقم ثابت — الرمز وحده يتغيّر."""

    def test_symbol_changes_but_number_never_does(self):
        config = AppConfig.get_solo()
        self.assertEqual(format_price(450000, config, "ar"), "450,000 ل.س")

        config.currency_code = "USD"
        config.currency_symbol = "$"
        config.save()
        config = AppConfig.get_solo()
        self.assertEqual(format_price(450000, config, "ar"), "450,000 $")

    def test_symbol_before_position(self):
        config = AppConfig.get_solo()
        config.currency_symbol = "$"
        config.currency_position = "before"
        config.save()
        self.assertEqual(format_price(1200, AppConfig.get_solo(), "ar"), "$ 1,200")
